...
describe("/api/submissions/[id]/analyze/[analysisId] Accept and Reject Routes", () => {
  ...
  describe("POST .../accept", () => {
    it("returns 401 when unauthenticated", async () => {
      ...
      const request = new Request(
        "http://localhost/api/submissions/sub-1/analyze/a-1/accept",
        {
          method: "POST",
        },
      );
    ...
    it("returns 200 with result when promotion succeeds", async () => {
      ...
      expect(promoteDraftToKnowledgeMock).toHaveBeenCalledWith(
        "user-123",
        "sub-1",
        "a-1",
        undefined,
    ...
    it("returns 400 when analysis cannot be accepted", async () => {
      ...
      expect(json.error).toContain(
        "Cannot accept analysis with status REJECTED",
      );
    });

    it("returns 404 when analysis is not found", async () => {
      promoteDraftToKnowledgeMock.mockRejectedValue(
        new Error("Analysis not found or unauthorized"),
      );

      const request = new Request(
        "http://localhost/api/submissions/sub-1/analyze/a-404/accept",
        {
          method: "POST",
        },
      );
  ...
  describe("POST .../reject", () => {
    it("returns 401 when unauthenticated", async () => {
      ...
      const request = new Request(
        "http://localhost/api/submissions/sub-1/analyze/a-1/reject",
        {
          method: "POST",
        },
      );
    ...
    it("returns 200 when rejection succeeds", async () => {
      ...
      expect(rejectDraftMock).toHaveBeenCalledWith("user-123", "sub-1", "a-1");
    });
